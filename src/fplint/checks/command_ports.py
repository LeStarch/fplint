...
class CommandPorts(CheckBase):
    ...
    def run(self, result: CheckResult, model: Topology, extras: List[str]) -> CheckResult:
        ...
        for component in model.get_comp_list():
            component_mappings[component.get_name()] = {key: None for key in self.CMD_TYPES.keys()}
            component_mappings[component.get_name()]["component"] = component
            for port in component.get_ports():
                ...
                if port_target_type != "Serial" and port_target_type != port_type:
                    result.add_problem("invalid-target-port-in-connection", "command port connected to invalid type",
                                       model, component, port)
                    continue
                target_comp_obj = get_comp_by_name(model, port_target_comp)
                target_port_obj = get_port_by_name(target_comp_obj, port_target_port, port.get_target_num())
                component_mappings[component.get_name()][port_type] = target_port_obj
        for _, values in component_mappings.items():
